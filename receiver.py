...
def setup_server_socket(port: int) -> socket.socket:
...
def receive_key_packet(server: socket.socket) -> bytes:
    """Accept a key connection and receive key_length + key + iv."""
...
def receive_data_packet(server: socket.socket) -> bytes:
    """Accept a data connection and receive length + ciphertext."""
    conn, _ = server.accept()
    with conn:
        conn.settimeout(TIMEOUT)
...
def main() -> None:
    ...
    with setup_server_socket(KEY_PORT) as key_server, setup_server_socket(DATA_PORT) as data_server:
